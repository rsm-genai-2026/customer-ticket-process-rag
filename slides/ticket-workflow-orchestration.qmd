...
invoke it. The script inside **calls an LLM** to do the work.

**Automation**

: A deterministic script that runs without an LLM. Rule-based code,
table lookups, templates. The orchestrator runs it; no agent needed.
...
## Where Does the AI Actually Live? {.content-slide}
...
The other eight live under `automations/` — receive, classify, escalate,
...
- Deterministic steps are trivially testable and give an auditable rationale
...
│   ├── README.md
│   ├── install.sh                # links into .claude/skills/
│   └── scripts/
...
├── receive-ticket/
│   ├── README.md                 # no SKILL.md, no install.sh
│   └── scripts/
