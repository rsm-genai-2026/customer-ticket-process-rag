...
The repo splits the ten workflow steps into two folders:

- **`skills/`** — the two LLM-based steps (`check-faq-resolution`,
  `investigate-specialist-solution`). Each has a `SKILL.md` that an LLM
...
uv run python automations/summarize-workflow-suite/scripts/summarize_workflow_suite.py --scenario-id human_expert_billing_api_502
```
