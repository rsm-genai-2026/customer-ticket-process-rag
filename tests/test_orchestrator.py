...
def test_normalize_submission_applies_defaults() -> None:
    ...
    form = orchestrator.normalize_submission(payload)
...
def test_build_rows_match_csv_contract() -> None:
    form = orchestrator.normalize_submission(
        {
    ...
    customer = orchestrator.build_customer_row(form, "WEB-CUST-TEST")
    ticket = orchestrator.build_ticket_row(form, "WEB-TEST", "WEB-CUST-TEST")
...
def test_prepare_run_data_copies_and_appends_rows(tmp_path: Path) -> None:
    ...
    paths = orchestrator.prepare_run_data(source, tmp_path / "wf-web-testcase", ticket, customer)
...
def test_render_index_includes_example_ticket_dropdown() -> None:
    html = orchestrator.render_index()
...
def test_process_submission_generates_customer_response(monkeypatch, tmp_path: Path) -> None:
    ...
    io_by_skill = {record["skill"]: record for record in result["stepIO"]}
...
def test_step_mode_runs_one_skill_at_a_time(tmp_path: Path) -> None:
    start = orchestrator.start_submission(
        {
            "requester_name": "Avery Chen",
            "requester_email": "avery@example.com",
            "company": "Northstar Retail",
    ...
    assert start["orchestrator"]["nextStep"] == "receive-ticket"
    ...
    first_step = orchestrator.process_step(
        {"workflow_run_id": start["workflowRunId"]},
        work_root=tmp_path,
    )

    assert first_step["orchestrator"]["lastStep"]["skill"] == "receive-ticket"
    assert first_step["orchestrator"]["nextStep"] == "classify-prioritize-ticket"
