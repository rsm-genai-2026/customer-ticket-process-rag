...
_MODULE_PATH = _REPO_ROOT / "automations" / "audit-ticket-process" / "scripts" / "audit_ticket_process.py"
...
def test_state_breaks_ties_on_equal_timestamps_by_step_order(tmp_path: Path) -> None:
    """When triage and FAQ rows share a timestamp (back-to-back skill runs
    inside one wall-clock second), the audit must still report the later
    logical step. Otherwise it routes back to ``check-faq-resolution``
    after the FAQ already ran."""

    data = _isolated_data(tmp_path)
    out = tmp_path / "out"
    out.mkdir()

    shared_ts = "2026-04-30T09:00:00+00:00"
    pl.DataFrame(
        [
            {
                "ticket_id": "TKT-X",
                ...
                "assigned_priority": "medium",
                "recommended_specialist_group": "identity_security",
                "target_first_response_at": shared_ts,
                "target_resolution_at": shared_ts,
                "classification_evidence": "test",
                "priority_reason": "test",
                "confidence_score": 0.9,
                "inputs_used": "x",
    ...
    ).write_csv(out / "triage_decisions.csv")
    pl.DataFrame(
        [
            {
                "ticket_id": "TKT-X",
                "created_at": shared_ts,  # same second as triage
                "skill_name": "check-faq-resolution",
    ...
    ).write_csv(out / "faq_decisions.csv")
